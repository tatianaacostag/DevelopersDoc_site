...
* _Reports_ > _Refunds y disputes_
...
1. When we are notified by the bank that a dispute process has been launched, you y your customer automatically receive an e-mail informing about that process.
...
2. Log into your PayU account. In the left menu, expand the _**Transacciones**_ menu y select _**Disputas**_.

![PrintScreen](/assets/Disputes/Disputes_02.png)

3. The _**Disputas**_ module opens, scroll down the page y locate the dispute you have opened. You can use the available filters to find your dispute if it is required.
...
5. In the pop up window, you can find the details about why your customer request the dispute process y the dead line to provide evidence to resolve the dispute. Click the _**Attach file**_ link to upload all the evidence you have.
...
6. Upload the evidence files to resolve the dispute y click _**Save evidence**_. It is mandatory to save the evidence first before send it to review.
...
The dispute case may result in won (no chargeback), Lost (chargeback) or refunded. In the case of refunds, the trade is who makes the return to the buyer y the bank does not generate the chargeback. Refer to [dispute states]({{< ref "Disputes.md#dispute-states" >}}) to know all the states of a dispute.
