...
## Requirements for Using Network Tokens
...
| `transaction > extraParameters > CONSENT_TRANSACTION_ID` | Alphanumeric | Max: 255 | The Network Payment Reference. Mandatory for subsequent recurring payments. Use the value returned in the initial transaction response. Refer to [Recurring Payments with Network Tokens](#recurring-payments-with-network-tokens) | No* |
...
## Recurring Payments with Network Tokens (Argentina)

PayU supports recurring billing models (subscriptions or automatic top-ups) using Network Tokens. This process links a sequence of transactions via a unique network reference, ensuring higher approval rates for automated charges and reducing the need for repeated security cryptograms after the initial setup.

{{% alert title="Important" color="warning"%}}

Recurring Payments with Network Tokens is only available in Argentina.

{{% /alert %}}

### 1. Initial Authorization (Start of Recurrence)

To start a recurring cycle, you must process an initial transaction using the Network Token and its corresponding Cryptogram (TAVV/CAVV).

* **Request:** Include `networkToken.tokenPan`, `networkToken.cryptogram`, and `networkToken.expiry`.

* **Response:** PayU will return a `CONSENT_TRANSACTION_ID` inside the `extraParameters` object. **You must store this value** in your database, as it serves as the Network Payment Reference for all future charges in this subscription.

### 2. Subsequent Authorizations (Recurring Charges)

For all future payments in the same subscription cycle, you do not need to generate a new cryptogram.

* **Request:** Include `networkToken.tokenPan` and `networkToken.expiry`. **Do not send the cryptogram parameter**.

* **Extra Parameter:** You must include `CONSENT_TRANSACTION_ID` within the `extraParameters` object, using the value stored from the initial transaction. Ensure `creditCard.processWithoutCvv2` is set to `true` in your request (this requires prior activation via your Sales Representative).

### Request Examples for Recurring Payments

{{< tabs tabTotal="2" tabID="2" tabName1="JSON" tabName2="XML" >}}
{{< tab tabNum="1" >}}
<br>

Initial Request (Includes `cryptogram`):
```JSON
"networkToken": {
    "tokenPan": "4097440000000004",
    "cryptogram": "11223344556677889900112233445566778899",
...
Subsequent Request (Includes `CONSENT_TRANSACTION_ID`)
```JSON
"transaction": {
    "networkToken": {
        "tokenPan": "4097440000000004",
