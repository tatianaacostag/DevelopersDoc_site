...
* *Refund*: this operation requests the reversal of a previously captured transaction. Refunds can be requested by the total or partial amount.
...
1. When a refund is requested by your customer, you need to request it using the API or SDK integration. The request requires the _Order Id_, and a refund reason.
...
- ```UNRESOLVED```: the transaction has been sent to PayU to approval. At this point, no transaction has been added to the order and when you consult it using the [Queries service]({{< ref "queries" >}} "Query Service"), the response only shows the transaction of the payment approval.
