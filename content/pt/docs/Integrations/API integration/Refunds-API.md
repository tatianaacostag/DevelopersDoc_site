...
| transaction > order > id | Number |  | Identifier of the order to be voided. | Sim |
...
| merchant > apiKey | Alfanumérico | Min:6 Max:32 | Senha fornecida pelo PayU. [Como faço para obter minha API key]({{< ref "integrations.html#api-key-and-api-login" >}}) | Sim |
| transaction |  |  | This object has the transaction data. | Sim |
| transaction > additionalValues > |  | 64 | Amount of the partial refund. This parameter and its values are mandatory when you want to perform a partial refund | Não |
| transaction > additionalValues > TX_VALUE | Alfanumérico | 64 | Amount of the transaction. Mandatory for partial refunds | Não |
| transaction > additionalValues > TX_VALUE > value | Number | 19 | Specifies the amount of the transaction. Mandatory for partial refunds | Não |
| transaction > additionalValues > TX_VALUE > currency | Alfanumérico | 3 | Código ISO da moeda. [Veja as moedas aceitas]({{< ref "response-codes-and-variables.html#accepted-currencies" >}}). Mandatory for partial refunds | Não |
| transaction > order |  |  | This object has the order data. | Sim |
| transaction > order > id | Number |  | Identifier of the order to be refunded. | Sim |
