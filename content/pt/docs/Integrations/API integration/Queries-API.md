...
tags: ["subtopic"]
---
Para integrar com a API de consultas, direcione sua solicitação para as seguintes URLs de acordo com seu ambiente.

{{% alert title="URL" color="info"%}}
* Teste: ```https://sandbox.api.payulatam.com/reports-api/4.0/service.cgi```
* Produção: ```https://api.payulatam.com/reports-api/4.0/service.cgi```
{{% /alert %}}

## Métodos disponíveis {#available-methods}
A API de Consultas inclui os seguintes métodos:
...
*Order Id* é um valor gerado pelo PayU para identificar todas as transações geradas para uma solicitação de pagamento realizada por seu cliente. Você pode usar o comando `ORDER_DETAIL` para consultar o status de uma ordem por seu identificador.

A seguir estão os exemplos de pedido e resposta para esta operação.
...
*Transaction Id* é um valor gerado por PayU para identificar uma transação gerada para uma ordem. Você pode usar o comando `TRANSACTION_RESPONSE_DETAIL` para consultar as informações de uma determinada transação.
...
Você pode usar `ORDER_DETAIL_BY_REFERENCE_CODE` para consultar o status de uma ordem por seu próprio identificador de ordem (referência).
...
## Ping
O método `PING` permite que você confirme a conexão com a nossa plataforma. A seguir estão os exemplos de pedido e resposta para este método.
